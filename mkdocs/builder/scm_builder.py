...
from mkdocs.builder.subsite import SubSite
...
class ScmBuilder():
    """Builds a Mkdocs site from a given version number.

    Note that there are limitations:

    * The mkdocs.yaml is not reloaded. At the moment, this is intentional, to avoid older plugins that
    ...
    def get_concrete_version(self, version_identifier: str):
        """Returns a concrete version number corresponding to the given version identifier.

        For Git, this resolves commit-ish's to commit hashes
            https://stackoverflow.com/a/23303550/6876989

        For Perforce, this resolves labels to their corresponding changelist.
        """
        raise NotImplementedError('Must be implemented by inheritor.')

    def unpack_version(self, config: MkDocsConfig, concrete_version: str):
        """Unpacks a SCM-backed repo into the location specified by config['docs_dir']."""
        raise NotImplementedError('Must be implemented by inheritor.')

    def get_subsite(self, version_identifier):
        ...
        """
        concrete_version = self.get_concrete_version(version_identifier)

        if concrete_version not in self.subsite_table:
            new_config = copy.deepcopy(self.config)
            new_config['docs_dir'] = tempfile.mkdtemp(prefix='mkdocs_src_')
            new_config['site_dir'] = tempfile.mkdtemp(prefix='mkdocs_srv_')
            self.unpack_version(new_config, concrete_version)

            self.subsite_table[concrete_version] = SubSite(new_config)
    ...
    def build(self, build_version=''):
        ...
        * MkdcosConfig.site_dir
        ...
        concrete_version = self.get_concrete_version(build_version)
        subsite = self.get_subsite(concrete_version)
        ...
        try:
            ...
            self.subsite_table[concrete_version].config = config.plugins.run_event('config', config)
            config = self.subsite_table[concrete_version].config
            ...
            if not self.live_server:  # pragma: no cover
                ...
                if self.dirty and self.site_directory_contains_stale_files(config.site_dir):
            ...
            for file in files.documentation_pages():
                ...
                self._populate_page(file.page, config, files, self.dirty)
            ...
            for template in config.theme.static_templates:
                self._build_theme_template(template, env, files, config, nav)

            for template in config.extra_templates:
                self._build_extra_template(template, files, config, nav)

            log.debug('Building markdown pages.')
            doc_files = files.documentation_pages()
            for file in doc_files:
                assert file.page is not None
                self._build_page(file.page, config, doc_files, nav, env, self.dirty)
    ...
    def _build_template(self, name: str, template: jinja2.Template, files: Files,
                        config: MkDocsConfig, nav: Navigation) -> str:
        """
        ...
        context = self.get_context(nav, files, config, base_url=base_url)
    ...
    def _build_theme_template(self, template_name: str, env: jinja2.Environment, files: Files,
                              config: MkDocsConfig, nav: Navigation) -> None:
        ...
        output = self._build_template(template_name, template, files, config, nav)
    ...
    def _build_extra_template(self, template_name: str, files: Files, config: MkDocsConfig,
                              nav: Navigation):
    ...
    def _populate_page(self, page: Page, config: MkDocsConfig, files: Files,
    ...
    def _build_page(
        ...
        try:
            ...
            context = self.get_context(nav, doc_files, config, page)
        ...
        def site_directory_contains_stale_files(self, site_directory: str) -> bool:
